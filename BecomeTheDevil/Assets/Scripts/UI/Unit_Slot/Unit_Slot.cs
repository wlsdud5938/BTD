...
public class Unit_Slot : MonoBehaviour
{
    public Stack<ItemObject> slot;       // 슬롯을 스택으로 만든다.
    public Text text;       // 아이템에 개수를 표현해줄 텍스트.
    public Sprite DefaultImg; // 슬롯에 있는 아이템을 다 사용할 경우 아무것도 없는 이미지를 넣어줄 필요가 있다.

    // 테스트용 아이템
    public ItemObject tmpItem;

    private Image ItemImg;
    private bool isSlot;     // 현재 슬롯이 비어있는지?

    public ItemObject ItemReturn() { return slot.Peek(); } // 슬롯에 존재하는 아이템이 뭔지 반환.
    public bool ItemMax(ItemObject item) { return ItemReturn().MaxCount > slot.Count; } // 겹칠수 있는 한계치를 넘으면. 
